...
def  create_model(word_to_id,train_data,dev_data,test_data,embedding_layer=None):
    """
    create ner model. word_to_id dict is neccessary. 
    Using Keras Default Embedding if embedding_layer = None.
    """
    ...
    #####
    # A test function is added here with a boolean value show to decide whether output ner result to a file
    #####
    show = True
    if show:
        amount = 0
...
def get_embedding(id_to_word):
    """
    This function will Load embedding from your pre-trained embedding file.
    A pickle file is defaulted.
    Utilize keras Embedding function to create embedding layer and return. 
    """
    EMBEDDING_DIM = 100
    MAX_SEQLEG = 100
    embeddings_index = {}
...
def testNer(X,Y,show=False):
    """
    This function focus on testing model by model.predict function.
    Measure criteria must be defined by actual condition and modified.
    """
